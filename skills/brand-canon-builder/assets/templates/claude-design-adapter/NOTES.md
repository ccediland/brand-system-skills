...
3. Run `/design-sync` from the kit dir; it reads `.design-sync/config.json`.
...
- **`cssEntry` (e.g. `ds.css`) is generated** by the build (concat of the token CSS partials) — don't author it by hand.
