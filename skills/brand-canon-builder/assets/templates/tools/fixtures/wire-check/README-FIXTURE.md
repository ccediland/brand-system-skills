...
| `fabricated-verbatim.md` | FAIL (exit 1) | `verbatim-miss` — quoted "her words" absent from the brief (the EV1-F03 class) · `wire-check` — declared verified:1 vs recomputed 0 (a hand-written check) |
| `markers-no-brief.md` | FAIL (exit 1) | `no-brief` — RATIFIED markers with no SIGNED BRIEF appendix (the fabrication surface) |
| `vocab-drift.md` | FAIL (exit 1) | the 6 EV1-F12 literal classes (`fidelity:reference` · `url:PENDIENTE` · `visibility:low-moderate` · `ingest:vector-trace` · `existing-material:pendiente` · `route-hint:font-match/trace`) + `n/a` + `born-gap` (posture `→GAP` with no GAPS row naming the field) |
