...
BRAND: Fixture Beta
OWNERS: demo-owner-b (all slots)   UNRESOLVED CONFLICTS: none
...
— HOW (grammar) —
  Schemes: single · contrast/imagery rule: none stated · mark-selection rule: symbol only
...
— POSTURE (guardrail layer) —
  profile:playful · visibility:low · audiences:fixture readers · regulatory:none · stance:neutral · never-topics:none · refusal-style:light deflection

— CORE-ASSET FIDELITY CONTRACT (this brand's must-haves) —
  symbol: GAP low-fi → fidelity-blocking
  NON-WAIVABLE even in v0/DEMO: the brand's PRIMARY-IDENTITY CARRIER(S) — visual mark (symbol) · graphic-code
...
— GAPS (client-language; builder formalizes to GAP-NNN) —
  ...
  tagline — why: owner asked for a draft · severity:NICE · provenance:handoff-deliberate · proposed: the PROPOSED draft above
