...
VERDICT / EXIT
  ...
  non-visual carrier → declared GAP ........ exit 0  (tracked horizon, never a false fail)
  outside-tolerance (degrade or GAP) ....... exit 1  (the build must act; evidence persisted)
...
TIERS = {
    "zero":    {"deltaE2000_max": 1.0, "ssim_min": 0.95, "mismatch_max": 0.02},  # primary-identity carrier + primary colours + NON-WAIVABLE
    "default": {"deltaE2000_max": 2.0, "ssim_min": 0.85, "mismatch_max": 0.05},
    "loose":   {"deltaE2000_max": 5.0, "ssim_min": 0.75, "mismatch_max": 0.10},  # gradients / illustration / incidental imagery
}
...
def load_deps():
    ...
    try:
        import cv2; mods["cv2"] = cv2
    except ImportError:
        missing.append(("cv2", "opencv-python-headless"))
    try:
        from skimage import color as skcolor
        from skimage.metrics import structural_similarity
        mods["skcolor"] = skcolor; mods["ssim"] = structural_similarity
    except ImportError:
        missing.append(("scikit-image", "scikit-image"))
    try:
        from PIL import Image; mods["Image"] = Image
    except ImportError:
...
def load_image(Image, np, path):
    ...
    if path.lower().endswith(".svg"):
        sys.stderr.write(f"fidelity-diff: {path} is SVG — rasterize it to PNG first "
...
def _de_field(skcolor, source, aligned):
    return skcolor.deltaE_ciede2000(skcolor.rgb2lab(source.astype("float64") / 255.0),
...
def _sane_homography(cv2, np, H, w, h):
    ...
    try:
        ...
        if a1 <= 0 or not (0.4 <= a1 / a0 <= 2.5):
            return False
        if wc.min() < -0.5 * max(w, h) or wc[:, 0].max() > 1.5 * w or wc[:, 1].max() > 1.5 * h:
            return False
        return True
    except cv2.error:
        return False
...
def coregister(cv2, np, skcolor, source, repro):
    ...
    h, w = source.shape[:2]
    repro_rs = cv2.resize(repro, (w, h), interpolation=cv2.INTER_AREA)  # shared grid for every candidate
    gs = cv2.cvtColor(source, cv2.COLOR_RGB2GRAY)
    gr = cv2.cvtColor(repro_rs, cv2.COLOR_RGB2GRAY)

    candidates = [(repro_rs, np.ones((h, w), bool), {"method": "resize-only"})]  # baseline to beat
    ...
    try:  # ORB + RANSAC homography
        ...
        if d1 is not None and d2 is not None and len(k1) >= 8 and len(k2) >= 8:
            ...
            if len(good) >= 8:
                ...
                if H is not None and inl is not None and int(inl.sum()) >= 8 and _sane_homography(cv2, np, H, w, h):
                    a = cv2.warpPerspective(repro_rs, H, (w, h))
                    m = cv2.warpPerspective(np.full((h, w), 255, "uint8"), H, (w, h)) > 0
                    candidates.append((a, m, {"method": "orb+ransac", "matches": len(good), "inliers": int(inl.sum())}))
    except cv2.error:
        pass
    ...
    try:  # phase correlation (translation) — reject the ~half-image garbage shift a flat image returns
        ...
        if abs(dx) <= 0.25 * w and abs(dy) <= 0.25 * h:
            ...
            candidates.append((a, m, {"method": "phase-correlation", "shift": [round(dx, 2), round(dy, 2)]}))
    except cv2.error:
        pass
    ...
    info["candidates"] = {c[2]["method"]: round(s, 4) for c, s in scored}  # auditable: every alignment's score
...
def measure(mods, source, repro):
    ...
    mismatch = float(np.mean((de > MISMATCH_DE)[mask])) if mask.any() else float(np.mean(de > MISMATCH_DE))
    ...
    if min(gs.shape) >= 7:
        ...
        ssim_val, ssim_gated = 1.0, False  # too small for a windowed structure metric → gate on ΔE only

    return aligned, mask, reg, de, {
...
def glyph_metrics(font_path, source_font_path, glyphs):
    ...
    try:
        ...
        sys.stderr.write("fidelity-diff: --font given but fontTools is missing: pip install fonttools\n")
        sys.exit(3)

    def load(p):
        f = TTFont(p)
        return f, (f.getBestCmap() or {}), f["hmtx"], f.getGlyphSet(), f["head"].unitsPerEm  # None cmap (symbol font) → {} → no comparable glyphs, not a crash
    ...
    for ch in glyphs:
        ...
        def npoints(glyphset, name):
            rp = RecordingPen(); glyphset[name].draw(rp)
            return sum(len(args[0]) if args and isinstance(args[0], (list, tuple)) else 1
                       for op, args in rp.value if op in ("lineTo", "curveTo", "qCurveTo", "moveTo"))
        ...
        per[ch] = {"advance_delta": round(adv_d, 4), "bbox_delta": round(bbox_d, 4),
                   "points_repro": pts_r, "points_source": pts_s}
    ...
    return {"per_glyph": per, "worst_advance_delta": round(worst_adv, 4),
            "worst_bbox_delta": round(worst_bbox, 4),
            "advance_tol": GLYPH_ADVANCE_TOL, "bbox_tol": GLYPH_BBOX_TOL, "pass": ok}, ok


def write_evidence(out_dir, treatment, payload, diff_png=None, mods=None, de=None, mask=None):
...
def main():
    ap = argparse.ArgumentParser(description="Measured reproduction-fidelity gate (MT-2).")
    ap.add_argument("--treatment", required=True, help="treatment id (the audit/fidelity/<id>/ slot)")
    ...
    ap.add_argument("--out", default="audit/fidelity", help="evidence root (default: audit/fidelity)")
    ap.add_argument("--tier", choices=list(TIERS), default="default", help="§2 layered tier")
    ap.add_argument("--core-color", action="store_true",
    ...
    ap.add_argument("--carrier", default="primary-identity carrier", help="carrier name (for the GAP note)")
    ap.add_argument("--degraded-to", default=None, help="resolution annotation if degraded to a lower method")
    ap.add_argument("--gap", default=None, help="GAP-NNN this treatment is logged under, if any")
    ap.add_argument("--font", default=None, help="reproduction font (optional glyph metrics)")
    ap.add_argument("--source-font", default=None, help="source font for the glyph comparison")
    ap.add_argument("--glyphs", default="AaGgQ0", help="glyphs to compare (default AaGgQ0)")
    args = ap.parse_args()
    ...
    if args.medium == "non-visual":
        ...
        payload = {
            "tool": "fidelity-diff.py", "treatment": args.treatment, "medium": "non-visual",
            "carrier": args.carrier,
            "verdict": f"non-visual carrier → declared fidelity GAP ({gap}) — tracked horizon",
            "resolution": "declared-GAP",
            "pass": True,
            "note": ("primary carrier is non-visual (no raster/vector reproduction to measure); "
        ...
        print(f"fidelity-diff: {args.treatment} — non-visual carrier → declared GAP ({gap}); PASS → {path}")
        sys.exit(0)

    if not args.source or not args.reproduction:
        sys.stderr.write("fidelity-diff: --source and --reproduction are required for a visual diff "
    ...
    if args.core_color:
        tier["deltaE2000_max"] = min(tier["deltaE2000_max"], TIERS["zero"]["deltaE2000_max"])
    ...
    if args.font:
        if not args.source_font:
            sys.stderr.write("fidelity-diff: --font needs --source-font to compare against.\n")
            sys.exit(2)
        glyph, glyph_ok = glyph_metrics(args.font, args.source_font, args.glyphs)

    within = (metrics["deltaE2000_mean"] <= tier["deltaE2000_max"]
              and (not metrics["ssim_gated"] or metrics["ssim"] >= tier["ssim_min"])  # SSIM N/A on a flat carrier
              and metrics["mismatch_fraction"] <= tier["mismatch_max"]
              and glyph_ok)

    if within:
        verdict, resolution, passing = "within-tolerance", "within-tolerance", True
    else:
        verdict = "outside-tolerance"
        if args.gap:
            # A DECLARED, tracked fidelity GAP (optionally after degrading) is a legitimate logged
            # outcome — never a tool false-fail. validate-audit.md §1/§2/§7b own the build-level block
            # (fidelity-blocking per the carrier's role); the tool only refuses to let it pass SILENTLY.
            ...
            resolution = "outside-tolerance → degrade to a lower method or log a GAP-NNN"
            passing = False

    payload = {
        "tool": "fidelity-diff.py", "treatment": args.treatment, "medium": "visual",
        "tier": args.tier, "registration": reg,
        "metrics": {k: metrics[k] for k in ("deltaE2000_mean", "deltaE2000_p95", "ssim", "ssim_gated", "mismatch_fraction")},
        "glyph": glyph,
        "thresholds": tier,
        "verdict": verdict, "resolution": resolution, "pass": bool(passing),
        "source": args.source, "reproduction": args.reproduction, "diffImage": "diff.png",
        "note": ("measured against the Stage-5 SOURCE capture — NOT a pixel-VRT against a reference "
                 "render (none exists; see validate-audit.md §3a)."),
    }
    path = write_evidence(args.out, args.treatment, payload, diff_png="diff.png",
                          mods=mods, de=de, mask=mask)

    msg = (f"fidelity-diff: {args.treatment} [{args.tier}{'/core' if args.core_color else ''}] "
    ...
    (print if passing else lambda m: sys.stderr.write(m + "\n"))(msg + f" → {path}")
