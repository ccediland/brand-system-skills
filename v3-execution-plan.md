...
This plan is self-contained. The completed v2 stress-test findings and the v3 research are distilled into the Decisions, Dead-ends, and phase descriptions below; their durable residue lives in `RESIDENT.md` (goals and decisions) and `CLAUDE.md` (technical methods, tooling, and gotchas). Work from those three repo documents, not from any working notes.
