...
| OI-K | **Ratification content-bind is value-bound, not slot-bound** (post-F3 fix-pack pre-merge verify, findings #1/#4, DOWNGRADED non-blocking). `audit-lint` R3's content-bind requires the cited `sources/ratification—<date>.md` to NAME the token's canonical value (strict matcher), but not that the record name THIS token's slot/path — so a record ratifying value X for slot A can be cited by another token (slot B) also carrying X, and a value the record names in a "rejected" block still matches. Closing it fully needs a record-shape convention (the record NAMES the slot near the value) = a possible contract touch. Detalle + repro en `verify-note-f3-fix-ratification-content-bind—2026-07-17.md` §C. | MAJOR | OPEN (residual declarado — gated en decisión del operador sobre formalizar la convención registro↔slot) |
